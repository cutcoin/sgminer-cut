...
__kernel void search(__global ulong *input, uint InputLen, __global uint4 *Scratchpad, __global ulong *states)
{
	...
	Scratchpad += (get_global_id(0) - get_global_offset(0)) * (SCR_SIZE / sizeof(Scratchpad[0]));
	...
	for(int i = 0; i < 0x4000; ++i)
	{
		...
		Scratchpad[(i << 3) + get_local_id(1)] = text;
	}
...
__kernel void search1(__global uint4 *Scratchpad, __global ulong *states)
{
  ...
  uint4 a_x = ((uint4 *)a)[0];
    ...
    c = Scratchpad[(a_x.s0 & 0x1FFFC0) >> 4];
    c = rotate(c, b_x.s0 & 31);
    c += Scratchpad[((a_x.s0 & 0x1FFFC0) >> 4) + 1];
    c = rotate(c, b_x.s1 & 31);
    c += Scratchpad[((a_x.s0 & 0x1FFFC0) >> 4) + 2];
    c = rotate(c, b_x.s2 & 31);
    c += Scratchpad[((a_x.s0 & 0x1FFFC0) >> 4) + 3];
    c = rotate(c, b_x.s3 & 31);
    
		c = AES_Round(AES0, AES1, AES2, AES3, c, a_x);

    Scratchpad[(a_x.s0 & 0x1FFFC0) >> 4] ^= rotate(b_x, a_x.s0 & 31) + c;
    Scratchpad[((a_x.s0 & 0x1FFFC0) >> 4) + 1] ^= rotate(b_x, a_x.s1 & 31) + c;
    Scratchpad[((a_x.s0 & 0x1FFFC0) >> 4) + 2] ^= rotate(b_x, a_x.s2 & 31) + c;
    Scratchpad[((a_x.s0 & 0x1FFFC0) >> 4) + 3] ^= rotate(b_x, a_x.s3 & 31) + c;

		uint4 tmp;
    tmp = Scratchpad[(c.s0 & 0x1FFFC0) >> 4];
    tmp = rotate(tmp, a_x.s0 & 31);
    tmp -= Scratchpad[((c.s0 & 0x1FFFC0) >> 4) + 1];
    tmp = rotate(tmp, a_x.s1 & 31);
    tmp -= Scratchpad[((c.s0 & 0x1FFFC0) >> 4) + 2];
    tmp = rotate(tmp, a_x.s2 & 31);
    tmp -= Scratchpad[((c.s0 & 0x1FFFC0) >> 4) + 3];
    tmp = rotate(tmp, a_x.s3 & 31);

    a_x += c * tmp;

    Scratchpad[(c.s0 & 0x1FFFC0) >> 4] ^= rotate(a_x, tmp.s0 & 31);
    Scratchpad[((c.s0 & 0x1FFFC0) >> 4) + 1] ^= rotate(a_x, tmp.s1 & 31);
    Scratchpad[((c.s0 & 0x1FFFC0) >> 4) + 2] ^= rotate(a_x, tmp.s2 & 31);
    Scratchpad[((c.s0 & 0x1FFFC0) >> 4) + 3] ^= rotate(a_x, tmp.s3 & 31);
...
__kernel void search2(__global uint4 *Scratchpad, __global ulong *states, __global uint *Branch0, __global uint *Branch1, __global uint *Branch2, __global uint *Branch3)
{
  ...
  Scratchpad += (get_global_id(0) - get_global_offset(0)) * (SCR_SIZE / sizeof(Scratchpad[0]));
	...
	for(int i = 0; i < 0x4000; ++i)
	{
		text ^= Scratchpad[(i << 3) + get_local_id(1)];
