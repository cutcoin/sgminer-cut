...
void cryptonight(uint8_t *Output, uint8_t *Input, uint32_t Length)
{
  ...
  uint32_t a[4], b[4];
	...
	a64[0] = CNCtx.State[0] ^ CNCtx.State[4];
	b64[0] = CNCtx.State[2] ^ CNCtx.State[6];
	a64[1] = CNCtx.State[1] ^ CNCtx.State[5];
	b64[1] = CNCtx.State[3] ^ CNCtx.State[7];
  ...
  memcpy(b, b64, 16);
	...
	{
		uint32_t c[4], tmp[4];
		memcpy(c, CNCtx.Scratchpad + ((a[0] & 0x1FFFC0) >> 3), 16);
    rotate4x32(c, b[0] & 31);
    add4x32(c, CNCtx.Scratchpad + ((a[0] & 0x1FFFC0) >> 3) + 2);
    rotate4x32(c, b[1] & 31);
    add4x32(c, CNCtx.Scratchpad + ((a[0] & 0x1FFFC0) >> 3) + 4);
    rotate4x32(c, b[2] & 31);
    add4x32(c, CNCtx.Scratchpad + ((a[0] & 0x1FFFC0) >> 3) + 6);
    rotate4x32(c, b[3] & 31);
    ...
    for(int j = 0; j < 4; ++j)
    {
      ...
      add4x32(tmp, c);
      xor4x32(CNCtx.Scratchpad + ((a[0] & 0x1FFFC0) >> 3) + j * 2, tmp);
    }

    memcpy(tmp, CNCtx.Scratchpad + ((c[0] & 0x1FFFC0) >> 3), 16);
    rotate4x32(tmp, a[0] & 31);
    sub4x32(tmp, CNCtx.Scratchpad + ((c[0] & 0x1FFFC0) >> 3) + 2);
    rotate4x32(tmp, a[1] & 31);
    sub4x32(tmp, CNCtx.Scratchpad + ((c[0] & 0x1FFFC0) >> 3) + 4);
    rotate4x32(tmp, a[2] & 31);
    sub4x32(tmp, CNCtx.Scratchpad + ((c[0] & 0x1FFFC0) >> 3) + 6);
    ...
    for(int j = 0; j < 4; ++j)
    {
      ...
      xor4x32(CNCtx.Scratchpad + ((b[0] & 0x1FFFC0) >> 3) + j * 2, c);
    }
